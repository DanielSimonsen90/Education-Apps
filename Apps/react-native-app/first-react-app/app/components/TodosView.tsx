...
export default function TodosView() {
    const [modal, triggerConfirm] = useConfirm("Are you sure, you want to clear all of your completed to-dos?", 
    ...
    const onDeselect = (e: PressEvent) => {

    }

    // const clearCompletedComponent = todosToClear > 0 && (<>
    //     <Button onPress={onDeselect} title=""/>
    ...
    const clearComponent = (<>
        <Button style={Styles.button} onPress={onClearCompletedPressed} title={`Clear ${selectedTodos ? 'Completed Todos' : 'List'}`} type="cancel" icon={{ name: 'delete', color: 'white' }} />
    </>)

    const todoItemsListView = todos.length > 0 && (
        todos.map((todo, i) => ( 
            <TodoListView key={i} todo={todo} onDelete={() => setTodos(v => v.filter(i => i != todo))} />
        ))
    ) || (  
